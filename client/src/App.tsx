...
function HomePage() {
  const { benchmarks: liveCards } = useLiveData();
  const cards: BenchmarkCard[] = liveCards && liveCards.length ? liveCards : benchmarks;
  return <>
    <main>
...
function ExplorerPage() {
  const { benchmarks: liveCards, loading: cardsLoading } = useLiveData();
  const cards: BenchmarkCard[] = liveCards && liveCards.length ? liveCards : benchmarks;
  ...
  const pageSize = 4;
  const datasetOptions = Array.from(new Set(cards.map((item) => item.dataset)));
  const taskOptions = Array.from(new Set(cards.map((item) => item.task)));
  const metricOptions = Array.from(new Set(cards.map((item) => item.metric)));
  ...
  }, [query, dataset, task, metric, framework, dateRange, sortBy, cards]);
  const totalPages = Math.max(1, Math.ceil(filtered.length / pageSize));
  const paged = filtered.slice((page - 1) * pageSize, page * pageSize);
  const selectedItems = cards.filter((item) => compare.includes(item.id));
...
function LeaderboardsPage() {
  const { leaderboards, loading: boardsLoading } = useLiveData();
  const boards = leaderboards?.names?.length ? leaderboards.names : ["ImageNet-1K", "COCO 2017", "SA-1B"];
  ...
  const activeRankings = leaderboards ? (leaderboards.boards[activeBoard] ?? rankings) : (leaderboardData[board] ?? rankings);
  const metricLabel = leaderboards ? (leaderboards.metrics[activeBoard] ?? "Score") : board === "COCO 2017" ? "mAP @ 50:95" : board === "SA-1B" ? "mIoU" : boardMetric;
  ...
  return <main className="subpage"><PageIntro eyebrow="THE LEADERBOARD" title={<>See who is <em>moving the line.</em></>} copy="A transparent view of the strongest reported results, ordered by the metrics that matter." action={<button className="button button-ghost" onClick={exportBoard}><Download size={16} /> Export board</button>} />
    <section className="leaderboard-section container"><div className="board-controls glass-panel reveal"><div><span className="control-label">DATASET</span><select value={activeBoard} onChange={(e) => { setBoard(e.target.value); setSelectedModel(null); }}>{boards.map((name) => <option key={name}>{name}</option>)}</select></div><div><span className="control-label">PRIMARY METRIC</span><select value={boardMetric} onChange={(e) => setBoardMetric(e.target.value)}><option>Top-1 accuracy</option><option>mAP @ 50:95</option><option>mIoU</option></select></div><div className="board-updated"><span className="live-pulse" /> Live board <small>{boardsLoading ? "Loading…" : "Updated with live data"}</small></div></div>{exportNotice && <div className="board-notice reveal is-visible"><CheckCircle2 size={15} /> Export prepared for {activeBoard} · {metricLabel}</div>}
